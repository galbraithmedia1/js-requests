...
const repeatMyParam = () => {
   axios.get(`http://localhost:3000/repeat/param2`)
   .then( (res)=> {
   console.log(res.data)
    const repeatTextPara = document.querySelector("#repeat-text")

    repeatTextPara.textContent = res.data;
...
// CODE HERE
const myQ = () => {
    axios.get("http://localhost:3000/query-test?car=honda" )
    .then( (res)=> {
         console.log(res)
    }).catch(err => console.log(err))
 }

document.getElementById('query-button').addEventListener('click', myQ)
